...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $departamento = isset($_POST['departamento']) ? mysqli_real_escape_string($conn, $_POST['departamento']) : '';
    $profesor_responsable = isset($_POST['profesor_responsable']) ? mysqli_real_escape_string($conn, $_POST['profesor_responsable']) : '';
    $trimestre = isset($_POST['trimestre']) ? mysqli_real_escape_string($conn, $_POST['trimestre']) : '';
    ...
    $organizador = isset($_POST['organizador']) ? mysqli_real_escape_string($conn, $_POST['organizador']) : '';
    $acompanantes = isset($_POST['acompanantes']) ? mysqli_real_escape_string($conn, $_POST['acompanantes']) : '';
    $ubicacion = isset($_POST['ubicacion']) ? mysqli_real_escape_string($conn, $_POST['ubicacion']) : '';
    $coste = isset($_POST['coste']) ? mysqli_real_escape_string($conn, $_POST['coste']) : '';
    $total_alumnos = isset($_POST['total_alumnos']) ? mysqli_real_escape_string($conn, $_POST['total_alumnos']) : '';
    $objetivo = isset($_POST['objetivo']) ? mysqli_real_escape_string($conn, $_POST['objetivo']) : '';
        ...
        <?php else: ?>
            <form method="POST" action="">
                ...
                <div>
                    ...
                    <input type="text" id="titulo" name="titulo" value="<?php echo escapar($row['titulo']); ?>">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" id="fecha_inicio" name="fecha_inicio" value="<?php echo escapar($row['fecha_inicio']); ?>">
                </div>
                <div>
                    <label for="hora_inicio">Hora Inicio:</label>
                    <input type="time" id="hora_inicio" name="hora_inicio" value="<?php echo escapar($row['hora_inicio']); ?>">
                </div>
                <div>
                    <label for="fecha_fin">Fecha Fin:</label>
                    <input type="date" id="fecha_fin" name="fecha_fin" value="<?php echo escapar($row['fecha_fin']); ?>">
                </div>
                <div>
                    <label for="hora_fin">Hora Fin:</label>
                    <input type="time" id="hora_fin" name="hora_fin" value="<?php echo escapar($row['hora_fin']); ?>">
                </div>
                <div>
                    <label for="organizador">Organizador:</label>
                    <input type="text" id="organizador" name="organizador" value="<?php echo escapar($row['organizador']); ?>">
                </div>
                <div>
                    <label for="acompanantes">Acompañantes:</label>
                    <textarea id="acompanantes" name="acompanantes"><?php echo escapar($row['acompanantes']); ?></textarea>
                </div>
                <div>
                    <label for="ubicacion">Ubicación:</label>
                    <input type="text" id="ubicacion" name="ubicacion" value="<?php echo escapar($row['ubicacion']); ?>">
                </div>
                <div>
                    <label for="coste">Coste:</label>
                    <input type="number" step="0.01" id="coste" name="coste" value="<?php echo escapar($row['coste']); ?>">
                </div>
                <div>
                    <label for="total_alumnos">Total Alumnos:</label>
                    <input type="number" id="total_alumnos" name="total_alumnos" value="<?php echo escapar($row['total_alumnos']); ?>">
                </div>
                <div>
                    <label for="objetivo">Objetivo:</label>
                    <textarea id="objetivo" name="objetivo"><?php echo escapar($row['objetivo']); ?></textarea>
                </div>
